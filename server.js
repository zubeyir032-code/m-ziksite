...
function parseProgress(dlEntry, text) {
  ...
  if (match) {
    dlEntry.progress = parseFloat(match[1]);
    dlEntry.status = 'downloading';
  }
}
...
app.post('/api/download', (req, res) => {
  ...
  dl.stdout.on('data', (data) => parseProgress(downloads[id], data.toString()));

  dl.stderr.on('data', (data) => parseProgress(downloads[id], data.toString()));
  ...
  dl.on('close', (code) => {
    if (code === 0) {
      ...
      const log = downloads[id].log || '';
      const lastLines = log.split('\n').filter(l => l).slice(-5).join(' | ');
      downloads[id].status = 'error';
      downloads[id].error = `yt-dlp çıkış kodu: ${code} — ${lastLines}`;
    }
  });
  ...
  try {
    ...
    dl.on('close', (code) => {
      ...
      if (code === 0 && filePath) {
        ...
        const dlEntry = downloads[id];
        const lastLines = (dlEntry?.log || '').split('\n').filter(l => l).slice(-3).join(' | ');
        downloads[id].status = 'error';
        item.status = 'hata';
        item.error = `İndirme hatası (kod: ${code}) — ${lastLines}`;
      }
      batch.completed++;
      onDone();
    });
...
// Python Scripts yolunu PATH'e ekle (yt-dlp için)
try {
  const pyScripts = require('child_process').execSync(
    'python3 -c "import sys,os; print(os.path.join(sys.base_prefix, \'Scripts\'))"',
    { encoding: 'utf8', timeout: 5000 }
  ).trim();
  if (pyScripts && !process.env.PATH.includes(pyScripts)) {
    process.env.PATH = pyScripts + path.delimiter + process.env.PATH;
  }
} catch (_) {}
// WinGet bağlantılarını PATH'e ekle (ffmpeg için)
try {
  ...
  if (fs.existsSync(wingetLinks) && !process.env.PATH.includes(wingetLinks)) {
    process.env.PATH = wingetLinks + path.delimiter + process.env.PATH;
  }
} catch (_) {}
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  const deps = [
    ...
    ['ffmpeg', ['-version'], 'Ses/video dönüştürücü'],
